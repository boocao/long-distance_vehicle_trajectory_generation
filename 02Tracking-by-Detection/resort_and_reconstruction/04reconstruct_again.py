...
from _kalman_utils.Kalman_filter import trajectory_denoising_by_Kalman
from _kalman_utils.RTS_smoother import trajectory_smoothing_by_RTS
...
def repair_coordinates_in_img_edge(per_id_data):
    ...
    middle_mean_data=middle_mean_data[middle_mean_data[:,2]>=200,:]
    middle_mean_data=middle_mean_data[middle_mean_data[:,2]<=(3840-200),:]
    ...
    per_id_data=per_id_data[per_id_data[:,2]>=(mean_w / 2)]
    per_id_data=per_id_data[per_id_data[:,2]<=(3840-mean_w / 2)]
    per_id_data=per_id_data[per_id_data[:,3]>=(mean_h / 2)]
    per_id_data=per_id_data[per_id_data[:,3]<=(2160-mean_w / 2)]
...
def smoothing_xy_by_SG(x,y,smoothing_window):
    smoothed_x = signal.savgol_filter(x, smoothing_window, 3)
    smoothed_y = signal.savgol_filter(y, smoothing_window, 3)
...
if __name__=='__main__':
    ...
    for i in range(int(new_data[:, 1].max())):
        ...
        # smoothed_x, smoothed_y = smoothing_xy_by_SG(per_id_data[:, 2], per_id_data[:, 3], smoothing_window=21)
        # smoothed_x, smoothed_y = trajectory_denoising_by_Kalman(per_id_data[:, 2], per_id_data[:, 3])
        smoothed_x, smoothed_y = trajectory_smoothing_by_RTS(per_id_data[:, 2], per_id_data[:, 3])

        # vehicle_class=0
        vehicle_class= Counter(per_id_data[:,-1]).most_common(1)[0][0]
    ...
    # print(sortData)
    save_path='./reconstruction_txts/' + filename[:7] + '_reconstructed_again.txt'
